 ...
 * @property {() => object} [getGates] resolved config carrying the two injection gates
 * @property {{ shouldInject?: (agentId: string, config: object) => boolean, hasUserSpoken?: Function, isActive?: Function }} [tracker]
...
export class BootInjector {
  ...
  render(agent) {
    ...
    if (agentId !== undefined && this.tracker?.shouldInject !== undefined
      && this.tracker.shouldInject(agentId, gates) !== true) {
      return undefined
    }
...
export function readOwnInjections(session, pluginName) {
  ...
  for (const seq of readSurfaceNodes(session) ?? []) {
    ...
    // and our delta marker. The digest nudge and recall nudge share this
    // plugin name but declare no `form`, so they never enter the ledger.
