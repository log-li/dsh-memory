...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 「当前激活会话」（与两道注入闸门同一口径）并提供 GET/POST 两个动作。
     */
      ...
      return React.createElement('div', { style: { padding: '8px 0' } },
        ...
        React.createElement('div', { style: rowStyle },
          React.createElement('div', null,
            ...
            React.createElement('div', { style: subStyle }, '开启后，每轮结束（空闲时）用模型判断本会话有没有值得长期保存的内容，值得才写回（查重 + 版本校验，与 memory_write 同一套引擎）。默认关闭——这是唯一会自作主张写记忆库的路径'),
          ),
          React.createElement(Toggle, {
