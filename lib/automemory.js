...
export class AutoMemory {
  ...
  eligibility() {
    ...
    if (this.options.isRoot !== undefined && this.isRoot() !== true) {
      return { run: false, reason: 'not a root session' }
    }
  ...
  /** Whether this agent is a root session (subagents never auto-extract). */
  isRoot() {
    try {
      return this.options.isRoot(this.agent) !== false
    } catch {
      return true
    }
  }
