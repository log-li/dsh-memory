...
export const Config = z.object({
  ...
  // Optional automemory (default off): two-stage extraction at the idle
  // boundary, writing through the same engine as `memory_write`.
...
export function apply(ctx, config = {}) {
  ...
  const activeSessionId = () => {
    for (const [sessionId, instance] of autoMemoryBySession) {
      if (instance.agent?.id !== undefined && tracker.isActive(instance.agent.id)) return sessionId
    }
    return undefined
  }
  ...
  // Automemory: one instance per live ROOT agent (default off). It only runs
  ...
  ctx.effect(() => {
    const stopCreated = ctx.on('agent/created', ({ agent }) => {
      ...
      agent.ctx.effect(() => {
        ...
        return () => {
          auto.dispose()
          if (typeof sessionId === 'string') autoMemoryBySession.delete(sessionId)
        }
      })
    })
    return () => stopCreated()
