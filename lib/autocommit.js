...
export class AutoCommitter {
   ...
   * @returns {'clean' | 'waiting' | 'committed' | 'skipped' | 'failed'}
   *   `committed` only when a commit actually landed — `memory_write` reports
   *   this back to the model instead of assuming it.
   */
  check(initial = false) {
    if (this.disposed || !this.gitOk || this.committing) return 'skipped'
    ...
    if (!initial && now - this.dirtySince < this.quietMs) return 'waiting'
